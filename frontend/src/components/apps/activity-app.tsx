...
import { activities, userById, type ActivityType } from "@/lib/mock";
...
export function ActivityApp() {
  ...
  const rows = activities.filter((a) => filter === "All" || a.type === filter);
          ...
          {FILTERS.map((f) => (
            <button
              ...
              className={cn(
                ...
                filter === f ? "bg-accent text-accent-foreground" : "text-muted hover:bg-hover",
              )}
            >
              {f}
            </button>
