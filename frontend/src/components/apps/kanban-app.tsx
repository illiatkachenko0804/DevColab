...
import { initialColumns, tasks as taskMap, userById, type Column, type Task } from "@/lib/mock";
...
function TaskCard({ task, dragging }: { task: Task; dragging?: boolean }) {
  return (
    <div
      className={cn(
        "rounded-[var(--radius-card)] border border-separator bg-surface p-3 shadow-[var(--shadow-card)]",
        dragging && "rotate-2 cursor-grabbing shadow-[var(--shadow-pop)]",
      )}
    >
...
function ColumnView({ column }: { column: Column }) {
  ...
  const accent =
    column.id === "done" ? "var(--success)" : column.id === "doing" ? "var(--app-projects)" : "var(--faint)";
  return (
    <div className="flex w-72 shrink-0 flex-col">
      <div className="mb-2 flex items-center gap-2 px-1">
        ...
        <button type="button" aria-label="Add task" className="ml-auto cursor-pointer text-faint hover:text-foreground">
      ...
      <SortableContext items={column.taskIds} strategy={verticalListSortingStrategy}>
        <div
          ...
          className={cn(
            ...
            isOver ? "border-accent bg-accent/5" : "border-separator/70",
          )}
        >
          {column.taskIds.map((id) => (
            <SortableCard key={id} task={taskMap[id]} />
          ))}
        </div>
      </SortableContext>
    </div>
...
export function KanbanApp() {
  ...
  const findColumn = (id: string) =>
    columns.find((c) => c.id === id) ?? columns.find((c) => c.taskIds.includes(id));
  ...
  const onDragEnd = (e: DragEndEvent) => {
    ...
    if (oldIndex !== newIndex && newIndex >= 0) {
      setColumns((cols) =>
        cols.map((c) => (c.id === col.id ? { ...c, taskIds: arrayMove(c.taskIds, oldIndex, newIndex) } : c)),
      );
    }
  };
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="flex h-12 shrink-0 items-center gap-2 border-b border-separator px-4">
        <span className="font-semibold">Q3 Roadmap</span>
        <span className="text-sm text-muted">· {Object.keys(taskMap).length} tasks</span>
        <button
          type="button"
          className="ml-auto flex cursor-pointer items-center gap-1.5 rounded-lg bg-accent px-3 py-1.5 text-sm font-medium text-accent-foreground transition hover:brightness-110"
        >
      ...
      <div className="flex-1 overflow-x-auto p-4 no-scrollbar">
        ...
        >
          ...
          <DragOverlay>{activeId ? <TaskCard task={taskMap[activeId]} dragging /> : null}</DragOverlay>
        </DndContext>
      </div>
    </div>
  );
